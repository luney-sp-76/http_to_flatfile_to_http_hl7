...
def validate_hl7_message(hl7_message):
    ...
    Returns:
        ...
        indicating whether the message is
        valid and an error message if applicable.
    """
...
def generate_ack(hl7_message, ack_type='AA', error_message=None):
    ...
    Args:
        ...
        error_message (str, optional): The error message to include
          in the ACK if the ACK type is not 'AA'. Defaults to None.

    Returns:
        str: The generated ACK message.
     Raises:
        ValueError: If the MSH segment
          of the HL7 message does not contain enough fields.
    """
    segments = hl7_message.strip().replace('\n', '\r').split('\r')
    msh_segment = segments[0].split('|')
    if len(msh_segment) < 10:
        raise ValueError(
            "Invalid MSH segment: not enough fields "
            "to extract message control ID")
    ...
    ack_message = '|'.join(
        ack_msh_segment) + '\r' + '|'.join(msa_segment) + '\r'
    return ack_message
...
def forward_to_remote_host(hl7_message, remote_host, remote_port):
    ...
    context.load_cert_chain(
        certfile='./keys/tcp_server-cert.pem',
        keyfile='./keys/tcp_server-key.pem'
    )
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sock:
        ...
        with context.wrap_socket(sock, server_side=True) as ssock:
            ...
            # Once socked is wrapped, reference the
            #  'ssock' var going forward rather than 'sock'
...
def send_to_http_server(hl7_message):
    ...
    # Presume encrypt both the contents of the hl7_message,
    # and the http connection down the line - need certs
    ...
    response = requests.post(
        'http://localhost:8080/http_hl7',
        data=hl7_message, headers={'Content-Type': 'text/plain'})
...
class HL7TCPHandler(socketserver.BaseRequestHandler):
    def handle(self):
        ...
        try:
            ...
            context = ssl.SSLContext(protocol=ssl.PROTOCOL_TLS_SERVER)
            context.load_cert_chain(
                certfile='./keys/tcp-server-cert.pem',
                keyfile='./keys/tcp-server-key.pem')
            ...
            print(
                f"Forwarded HL7 message to HTTP server. Response: "
                f"{http_response}"
            )
